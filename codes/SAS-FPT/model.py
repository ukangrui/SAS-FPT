...
class TimeAwareMultiHeadAttention(torch.nn.Module):
    ...
    def __init__(self, hidden_size, head_num, dropout_rate, dev):
        ...
        self.hidden_size = hidden_size # 10 is week emb size
...
class TiSASRec(torch.nn.Module): # similar to torch.nn.MultiheadAttention
    def __init__(self, user_num, item_num, time_num, args, num_weeks = 4):
        ...
        self.item_emb = torch.nn.Embedding(self.item_num+1, args.hidden_units - 50, padding_idx=0)
        self.item_emb_dropout = torch.nn.Dropout(p=args.dropout_rate)
        self.user_emb = torch.nn.Embedding(self.user_num+1, 25)
        ###############
        self.week_emb = torch.nn.Embedding(num_weeks, 25)
        ...
        self.time_matrix_K_emb = torch.nn.Embedding(args.time_span+4, args.hidden_units)
        self.time_matrix_V_emb = torch.nn.Embedding(args.time_span+4, args.hidden_units)
        ...
        ##################################################
        self.week_linear = torch.nn.Linear(args.hidden_units, args.hidden_units)
        ...
        for _ in range(args.num_blocks):
            ...
            new_fwd_layernorm = torch.nn.LayerNorm(args.hidden_units , eps=1e-8) #### 10 is week emb size
            self.forward_layernorms.append(new_fwd_layernorm)

            new_fwd_layer = PointWiseFeedForward(args.hidden_units, args.dropout_rate) #### 10 is week emb size
            self.forward_layers.append(new_fwd_layer)

            # self.pos_sigmoid = torch.nn.Sigmoid()
            # self.neg_sigmoid = torch.nn.Sigmoid()

    def seq2feats(self, user_ids, log_seqs, time_matrices,weeks):
        self.sample_num = 100
        user_ids = user_ids -1
        user_embs = self.user_emb(torch.LongTensor(user_ids).to(self.dev)).unsqueeze(1).repeat(1, 50, 1)
        seqs = self.item_emb(torch.LongTensor(log_seqs).to(self.dev))
        seqs *= self.item_emb.embedding_dim ** 0.5
        seqs = self.item_emb_dropout(seqs)
        ###############################

        weeks = self.week_emb(torch.LongTensor(weeks).to(self.dev))
        seqs = torch.cat([seqs, user_embs, weeks], dim=-1)
    ...
    def forward(self, user_ids, log_seqs, time_matrices, pos_seqs, neg_seqs,weeks): # for training
        log_feats = self.seq2feats(user_ids, log_seqs, time_matrices,weeks)
        #weeks = torch.LongTensor(weeks).to(self.dev)
        #weeks = 0.01 * self.week_emb(weeks)


        #log_feats = torch.cat([log_feats, weeks], dim=-1)
        #log_feats = self.week_linear(log_feats)
        pos_embs = self.item_emb(torch.LongTensor(pos_seqs).to(self.dev))
        neg_embs = self.item_emb(torch.LongTensor(neg_seqs).to(self.dev))
        pos_weeks = self.week_emb(torch.LongTensor(weeks).to(self.dev))
        neg_weeks = self.week_emb(torch.LongTensor(weeks).to(self.dev))

        user_ids = user_ids -1
        user_embs = self.user_emb(torch.LongTensor(user_ids).to(self.dev)).unsqueeze(1).repeat(1, 50, 1)

        pos_embs = torch.cat([pos_embs,user_embs, pos_weeks], dim=-1)
        neg_embs = torch.cat([neg_embs,user_embs, neg_weeks], dim=-1)
    ...
    def predict(self, user_ids, log_seqs, time_matrices, item_indices,weeks): # for inference
        log_feats = self.seq2feats(user_ids, log_seqs, time_matrices,[weeks])
        final_feat = log_feats[:, -1, :][:, :75] # only use last QKV classifier, a waste
        item_embs = self.item_emb(torch.LongTensor(item_indices).to(self.dev)) # (U, I, C)
        user_ids = user_ids -1
        user_embs = self.user_emb(torch.LongTensor(user_ids).to(self.dev)).repeat(101, 1)
        item_embs = torch.cat([item_embs, user_embs], dim=-1)
    ...
    def sample_loss(self,user_cluster):
        user_cluster = torch.LongTensor(user_cluster).to(self.dev)
        user_idx_group1 = torch.randperm(self.user_num)[:self.sample_num].to(self.dev)
        user_idx_group2 = torch.randperm(self.user_num)[:self.sample_num].to(self.dev)
        user_emb_group1 = self.user_emb(user_idx_group1)
        user_emb_group2 = self.user_emb(user_idx_group2)
        label_emb_user = user_cluster[user_idx_group1] == user_cluster[user_idx_group2]
        map  = {
            True: 1,
            False: -1
        }
        label_emb_user = torch.tensor([map[i.cpu().item()] for i in label_emb_user],dtype=torch.float32,device=self.dev)
        return (user_emb_group1,user_emb_group2,label_emb_user)
