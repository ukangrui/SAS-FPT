...
def get_week_number(timestamp):
    ...
    # Return the week number (0-51)
    if week_number - 1 < 12:
        return 0
    elif week_number -1 >= 12 and week_number-1 <25:
        return 1
    elif week_number -1 >= 25 and week_number-1 < 38:
        return 2
    else:
        return 3
...
def Relation(user_train, usernum, maxlen, time_span):
    ...
    np.save('data/span_list.npy',np.array(span_list))
    return data_train
...
def sample_function(user_train, usernum, itemnum, batch_size, maxlen, relation_matrix, result_queue, SEED):
    def sample(user):
        ...
        weeks = np.zeros([maxlen], dtype=np.int32)
        ...
        return (user, seq, time_seq, time_matrix, pos, neg,weeks)
...
def data_partition(fname):
    ...
    for line in f:
        ...
        u = int(u)
        i = int(i)
        week = get_week_number(int(timestamp))
        timestamp = float(timestamp)
        time_set.add(timestamp)
        User[u].append([i, timestamp,week])
    f.close()
...
def evaluate(model, dataset, args):
    ...
    for u in users:
        ...
        idx = args.maxlen - 1
        weeks = np.zeros([args.maxlen], dtype=np.int32)
        
        seq[idx] = valid[u][0][0]
        time_seq[idx] = valid[u][0][1]
        weeks[idx] = valid[u][0][2]
        idx -= 1
        ...
        predictions = -model.predict(*[np.array(l) for l in [[u], [seq], [time_matrix],item_idx,weeks]])
